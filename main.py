...
for channel_user, message in channel_messages_array:
    try:
        ...
        _, _, matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality, matches_tuic, matches_hysteria, matches_juicity = find_matches(
            config_text_content)
...
for channel_user, messages in channel_check_messages_array:
    ...
    for message in messages:
        try:
            ...
            _, _, matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality, matches_tuic, matches_hysteria, matches_juicity = find_matches(
                config_text_content)
            total_config = total_config + len(matches_shadowsocks) + len(matches_trojan) + len(matches_vmess) + len(
                matches_vless) + len(matches_reality) + len(matches_tuic) + len(matches_hysteria) + len(matches_juicity)
...
for url in array_url:
    try:
        ...
        if tg_user not in ['proxy', 'img', 'emoji', 'joinchat'] and '+' not in tg_user and '-' not in tg_user and len(
                tg_user) >= 5:
            tg_user = ''.join(
                [element for element in list(tg_user) if element in string.ascii_letters + string.digits + '_'])
...
for channel, messages in new_channel_messages:
    ...
    for url in new_array_url:
        try:
            tg_user = tg_username_extract(url)
            if tg_user not in ['proxy', 'img', 'emoji',
                               'joinchat'] and '+' not in tg_user and '-' not in tg_user and len(tg_user) >= 5:
                tg_user = ''.join(
                    [element for element in list(tg_user) if element in string.ascii_letters + string.digits + '_'])
...
# Separate encoded and unencoded strings
channel_decoded_contents = list(
    map(lambda element: (element[0], decode_string(element[1])), channel_array_links_content))
...
for url_link, content in array_links_content_decoded:
    ...
    content_merged = "\n".join(content)
    match_user, match_url, match_socks, match_trojan, match_vmess, match_vless, match_reality, match_tuic, match_hysteria, match_juicity = find_matches(
        content_merged)

    if len(match_socks) + len(match_trojan) + len(match_vmess) + len(match_vless) + len(match_reality) + len(
            match_tuic) + len(match_hysteria) + len(match_juicity) != 0:
            ...
            if config.startswith('vless'):
                ...
                for pair in array_params_input:
                    try:
                        ...
                        key = re.sub(r"servicename", "serviceName", re.sub(r"headertype", "headerType",
                                                                           re.sub(r"allowinsecure", "allowInsecure",
                                                                                  key.lower()), ), )
...
def remove_duplicate(shadow_array, trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array,
                     juicity_array, vmess_decode_dedup=True):
...
def modify_config(shadow_array, trojan_array, vmess_array, vless_array, reality_array, tuic_array, hysteria_array,
                  check_port_connection=True):
    # Checkout connectivity and modify title and protocol type address and resolve IP address
    shadow_array, shadow_tls_array, shadow_non_tls_array, shadow_tcp_array, shadow_ws_array, shadow_http_array, shadow_grpc_array = check_modify_config(
        array_configuration=shadow_array, protocol_type="SHADOWSOCKS", check_connection=check_port_connection)
    trojan_array, trojan_tls_array, trojan_non_tls_array, trojan_tcp_array, trojan_ws_array, trojan_http_array, trojan_grpc_array = check_modify_config(
        array_configuration=trojan_array, protocol_type="TROJAN", check_connection=check_port_connection)
    vmess_array, vmess_tls_array, vmess_non_tls_array, vmess_tcp_array, vmess_ws_array, vmess_http_array, vmess_grpc_array = check_modify_config(
        array_configuration=vmess_array, protocol_type="VMESS", check_connection=check_port_connection)
    vless_array, vless_tls_array, vless_non_tls_array, vless_tcp_array, vless_ws_array, vless_http_array, vless_grpc_array = check_modify_config(
        array_configuration=vless_array, protocol_type="VLESS", check_connection=check_port_connection)
    reality_array, reality_tls_array, reality_non_tls_array, reality_tcp_array, reality_ws_array, reality_http_array, reality_grpc_array = check_modify_config(
        array_configuration=reality_array, protocol_type="REALITY", check_connection=check_port_connection)
    tuic_array, _, _, _, _, _, _ = check_modify_config(array_configuration=tuic_array, protocol_type="TUIC",
                                                       check_connection=False)
    hysteria_array, _, _, _, _, _, _ = check_modify_config(array_configuration=hysteria_array, protocol_type="HYSTERIA",
                                                           check_connection=False)
    ...
    for array in [shadow_non_tls_array, trojan_non_tls_array, vmess_non_tls_array, vless_non_tls_array,
...
configs_list_array = [array_shadowsocks, array_trojan, array_vmess, array_vless, array_reality, array_tuic,
                      array_hysteria, matches_shadowsocks, matches_trojan, matches_vmess, matches_vless,
                      matches_reality, matches_tuic, matches_hysteria, raw_matches_shadowsocks, raw_matches_trojan,
                      raw_matches_vmess, raw_matches_vless, raw_matches_reality, raw_matches_tuic, raw_matches_hysteria,
                      channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless,
...
array_shadowsocks, array_trojan, array_vmess, array_vless, array_reality, array_tuic, array_hysteria, array_juicity = remove_duplicate(
    array_shadowsocks, array_trojan, array_vmess, array_vless, array_reality, array_tuic, array_hysteria, array_juicity)
matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality, matches_tuic, matches_hysteria, matches_juicity = remove_duplicate(
...
raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality, raw_matches_tuic, raw_matches_hysteria, raw_matches_juicity = remove_duplicate(
    raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality,
    raw_matches_tuic, raw_matches_hysteria, raw_matches_juicity)
channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless, channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, channel_matches_juicity = remove_duplicate(
    channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless,
    channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, channel_matches_juicity)
...
raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality, raw_matches_tuic, raw_matches_hysteria, raw_matches_tls, raw_matches_non_tls, raw_matches_tcp, raw_matches_ws, raw_matches_http, raw_matches_grpc = modify_config(
    raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality,
    raw_matches_tuic, raw_matches_hysteria, check_port_connection=False)
channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless, channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, channel_matches_tls, channel_matches_non_tls, channel_matches_tcp, channel_matches_ws, channel_matches_http, channel_matches_grpc = modify_config(
    channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless,
    channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, check_port_connection=True)
...
array_shadowsocks_channels, array_trojan_channels, array_vmess_channels, array_vless_channels, array_reality_channels, array_tuic_channels, array_hysteria_channels, array_juicity_channels = remove_duplicate(
    array_shadowsocks_channels, array_trojan_channels, array_vmess_channels, array_vless_channels,
    array_reality_channels, array_tuic_channels, array_hysteria_channels, array_juicity_channels,
    vmess_decode_dedup=False)
channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless, channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, channel_matches_juicity = remove_duplicate(
    channel_matches_shadowsocks, channel_matches_trojan, channel_matches_vmess, channel_matches_vless,
    channel_matches_reality, channel_matches_tuic, channel_matches_hysteria, channel_matches_juicity,
    vmess_decode_dedup=False)
...
matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality, matches_tuic, matches_hysteria, matches_juicity = remove_duplicate(
    matches_shadowsocks, matches_trojan, matches_vmess, matches_vless, matches_reality, matches_tuic, matches_hysteria,
    matches_juicity, vmess_decode_dedup=False)
raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality, raw_matches_tuic, raw_matches_hysteria, raw_matches_juicity = remove_duplicate(
    raw_matches_shadowsocks, raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality,
    raw_matches_tuic, raw_matches_hysteria, raw_matches_juicity, vmess_decode_dedup=False)
...
# Remove Duplicate Configurations
array_list_configurations = [array_shadowsocks, array_trojan, array_vmess, array_vless, array_reality, array_tuic,
                             array_hysteria, matches_shadowsocks, matches_trojan, matches_vmess, matches_vless,
                             matches_reality, matches_tuic, matches_hysteria, raw_matches_shadowsocks,
                             raw_matches_trojan, raw_matches_vmess, raw_matches_vless, raw_matches_reality,
                             raw_matches_tuic, raw_matches_hysteria, channel_matches_shadowsocks,
                             channel_matches_trojan, channel_matches_vmess, channel_matches_vless,
                             channel_matches_reality, channel_matches_tuic, channel_matches_hysteria]
array_removed_duplicate_list_configurations = list()
...
datetime_update_str = datetime_update.strftime(
    "\U0001F504 LATEST-UPDATE \U0001F4C5 %a-%d-%B-%Y \U0001F551 %H:%M").upper()
# Define update time based on protocol type
reality_update, vless_update, vmess_update, trojan_update, shadowsocks_update = create_title(datetime_update_str,
                                                                                             port=1080)

# Define develooper sign
dev_sign = "\U0001F468\U0001F3FB\u200D\U0001F4BB DEVELOPED-BY Amirwopi \U0001F4CC FOLLOW-CONTACT Amirwopi_fox"
# Define develooper based on protocol type
reality_dev_sign, vless_dev_sign, vmess_dev_sign, trojan_dev_sign, shadowsocks_dev_sign = create_title(dev_sign,
                                                                                                       port=8080)

# Define Advertisement row
adv_bool = False
adv_sign = "\U0001F381 یه دونیتمون نشه؟ \U00002764\U0000FE0F DARAMET.COM/Amirwopi"
# Define develooper based on protocol type
reality_adv_sign, vless_adv_sign, vmess_adv_sign, trojan_adv_sign, shadowsocks_adv_sign = create_title(adv_sign,
                                                                                                       port=2080)
